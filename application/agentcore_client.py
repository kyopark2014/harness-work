...
_HARNESS_SYSTEM_PROMPT_BASE = (
    ...
    "- Markdown(`.md`) / JSON(`.json`) / CSV(`.csv`)은 `viewer_url`이 있으면 **viewer_url을 우선** 안내하세요 "
    "(앱 viewer). 필요 시 CloudFront `url`(raw)도 함께 안내해도 됩니다.\n"
    "- 그 외 파일은 CloudFront `url`을 안내하세요.\n"
    ...
    "- MCP `share_artifact` / `artifact-share`는 이 프로젝트에 없습니다. skill로만 공유하세요.\n"
    "- 일반 다운로드는 harness-work의 `S3_BUCKET` / `SHARING_URL`을 사용하세요. "
    "Markdown/JSON/CSV viewer만 `APP_URL` 기반 `viewer_url`을 사용합니다.\n"
    "\n"
    "## Agent Workflow\n"
    "1. 사용자 입력을 받는다\n"
    "2. 문서 작성 등은 필요 시 **skill**(docx/pptx/doc-sharing 등)을 ``skills``로 로드하고, "
    ...
    "Markdown/JSON/CSV은 viewer_url 우선)\n"
...
def build_harness_system_prompt(actor_id: str | None = None) -> list[dict]:
    ...
    if aid:
        ...
        text += (
            ...
            "\n"
            "산출물 생성 후 반드시 **doc-sharing** skill로 공유하세요:\n"
            f"  `python3 /tmp/doc-sharing/scripts/share_artifact.py "
            f"--filepath '{artifacts_dir}/파일명' --actor-id \"{aid}\"`\n"
            "반환된 URL을 사용자에게 전달하세요 (Markdown/JSON/CSV는 viewer_url 우선). "
            "로컬 경로만 알려주지 마세요.\n"
            "MCP 도구 ``retrieve``를 호출할 때 actor_id 인자에는 반드시 "
            f"위 값(\"{aid}\")을 그대로 사용하세요. "
