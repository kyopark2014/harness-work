...
def ingest_rag_upload(
    ...
    *,
    ...
    metadata_result = utils.upload_to_s3(
        metadata_bytes, metadata_file_name, user_id=user_id
    )
    if not metadata_result:
        raise RagServiceError(
            500, "Failed to upload Knowledge Base metadata file to S3"
        )

    sync_result = utils.sync_data_source()
    if not sync_result:
        raise RagServiceError(
            500, "File uploaded but Knowledge Base sync failed"
        )
