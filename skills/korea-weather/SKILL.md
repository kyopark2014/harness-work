...
| 스크립트 | 용도 |
| --- | --- |
| `/home/.agents/skills/s3/korea-weather/scripts/recall_home_location.py` | 지역명 미지정 시 memory에서 집/거주 주소 조회 |
| `/home/.agents/skills/s3/korea-weather/scripts/get_weather.py` | 기상청 동네예보·현재날씨·대기질 조회 |
...
```bash
SKILL_SCRIPTS="/home/.agents/skills/s3/korea-weather/scripts"
if [ ! -f "$SKILL_SCRIPTS/get_weather.py" ]; then
  SKILL_SCRIPTS="$WORKING_DIR/skills/korea-weather/scripts"
fi
```
...
1. 지역명이 없으면 **먼저** memory로 집/거주 주소를 조회하세요.
   ```bash
   python /home/.agents/skills/s3/korea-weather/scripts/recall_home_location.py
   ```
...
```bash
SCRIPTS=/home/.agents/skills/s3/korea-weather/scripts

# 1) memory에서 집/거주 주소
loc=$(python "$SCRIPTS/recall_home_location.py")

# 2) 주소가 있으면 그 지역으로 날씨 조회
if [ "$loc" != "LOCATION_NOT_FOUND" ]; then
  python "$SCRIPTS/get_weather.py" "$loc"
else
...
```bash
python /home/.agents/skills/s3/korea-weather/scripts/recall_home_location.py --json
```

### 현재 위치 별칭(스크립트 내부 memory→IP)

```bash
python /home/.agents/skills/s3/korea-weather/scripts/get_weather.py "현재위치"
```

### 지역명 지정

```bash
python /home/.agents/skills/s3/korea-weather/scripts/get_weather.py "서울 서초구 반포3동"
python /home/.agents/skills/s3/korea-weather/scripts/get_weather.py "부산 해운대"
python /home/.agents/skills/s3/korea-weather/scripts/get_weather.py "제주"
```

### 예보구역 stnId

```bash
python /home/.agents/skills/s3/korea-weather/scripts/get_weather.py --stnid 109
```
...
SCRIPTS = "/home/.agents/skills/s3/korea-weather/scripts"
if not os.path.isfile(os.path.join(SCRIPTS, "get_weather.py")):
    SCRIPTS = os.path.join(os.environ.get("WORKING_DIR", "/app"), "skills", "korea-weather", "scripts")

RECALL = os.path.join(SCRIPTS, "recall_home_location.py")
WEATHER = os.path.join(SCRIPTS, "get_weather.py")
...
- `requests`, `beautifulsoup4` (Harness runtime에 포함; **soupsieve 불필요** — 스크립트는 CSS selector를 쓰지 않음)
- API 키 불필요
- memory 조회: AgentCore memory / `AGENTCORE_USER_ID` 환경변수(선택)
...
반드시 `/home/.agents/skills/s3/korea-weather/scripts/...` 절대 경로를 사용하세요.
`$WORKING_DIR/skills/...` 또는 상대경로 `skills/...`는 Harness S3 마운트에서 존재하지 않습니다.
