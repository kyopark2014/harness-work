...
DEFAULT_DB = PROJECT_ROOT / "application" / "data" / "tasks.db"
...
DEFAULT_APP_CONFIG = PROJECT_ROOT / "application" / "config.json"
...
def resolve_tasks_db_for_user(user_id: str) -> Path:
    ...
    working = PROJECT_ROOT / "application" / "data" / "users" / f"{segment}.db"
...
def session_storage_dir() -> Path:
    load_env()
    default = PROJECT_ROOT / "application" / ".session_storage"
...
def load_app_config() -> dict[str, Any]:
    """Load application/config.json only (no other repos)."""
